@echo off
REM Stream Deck-knop: vraagt de backend op de laptop om de laptop zelf te herstarten.
call "%~dp0config.bat"
...
set RESPONSE_FILE=%TEMP%\dashboard_restart_laptop.json
for /f "tokens=* delims=" %%a in ('curl -s -m 5 -o "%RESPONSE_FILE%" -w "%%{http_code}" -X POST -H "X-Actions-Secret: %ACTIONS_SECRET%" %BACKEND_URL%/api/actions/restart-laptop') do set HTTP_CODE=%%a

if "%HTTP_CODE%"=="200" (
    echo [Dashboard] Gelukt: laptop herstart over 10 seconden.
